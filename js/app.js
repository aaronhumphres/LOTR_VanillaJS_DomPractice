...
function forgeTheFellowShip() {
	...
	for (let i = 0; i < 4; i++) {
		alert(hobbitsArray[0].innerText + " has been added to the Fellowship.")
...
function itsDangerousToGoAlone() {
	...
	const ringBearers = fellowship
	mordor.appendChild(ringBearers[0])
	mordor.appendChild(ringBearers[0])
...
function thereAndBackAgain() {
	...
	const theShire = document.querySelector('#The-Shire')
	const getallHobbits = document.querySelectorAll('.hobbit')
	const ulHobbits = document.querySelector("#hobbits")

	for (let i = 0; i < getallHobbits.length; i++) {
		ulHobbits.appendChild(getallHobbits[i])
	}
